...
// في بداية server.js بعد mongoose.connect مباشرة، أضف هذا:
...
// هذا التأكد أن السيرفر ما يموتش لو فشل الاتصال
app.use((req, res, next) => {
    // إذا كان اتصال MongoDB لم يكتمل بعد، انتظر
    if (mongoose.connection.readyState !== 1) {
        return res.status(503).json({ 
            error: 'الخادم في طور الاتصال بقاعدة البيانات، حاول مرة أخرى بعد قليل' 
        });
    }
    next();
});
