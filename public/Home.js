document.addEventListener('DOMContentLoaded', async function() {
    // الرابط الرسمي لموقعك (غيّره مرة واحدة لو حبيت تنقله في المستقبل)
    const BASE_URL = 'https://schoolx-five.vercel.app';

    // دالة جلب البيانات موحدة ومضمونة 100%
    ...
    document.getElementById('search-form')?.addEventListener('submit', function(e) {
        ...
        const student = students.find(s => s.fullName.includes(name) && s.id === id);
    ...
    function renderStudentResult(student, resultBody, violationsBody) {
        ...
        const values = [student.fullName, student.id, ...student.subjects.map(s => s.grade || 0)];
        ...
        const studentVios = violations.filter(v => v.studentId === student.id);
